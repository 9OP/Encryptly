...
import { UserInfo, FileMetadata, AppData, StorageQuota } from "../models";
...
export const getDriveId = async (token: string): Promise<string> => {
  const res = await fetch("https://www.googleapis.com/drive/v3/files/root", {
    method: "GET",
    headers: { Authorization: `Bearer ${token}` },
  });
  const json = await res.json();
  if (res.status === 404) {
    // Workaround to get the root id
    // message: `"File not found: 0AIBWSGArhz4WUk9PVA."`
    const rootId = json["error"]["message"].split(" ").at(-1).slice(0, -1);
    return rootId;
  }
  throw new Error(json["error"]["message"]);
};

export const createFolder = async (token: string, name: string, parent: string): Promise<any> => {
...
export const moveFile = async (token: string, fileId: string, parentId: string): Promise<any> => {
  const res = await fetch(
...
const uploadFileMetadata = async (token: string, parent: string, name: string): Promise<string> => {
  ...
  const res = await fetch("https://www.googleapis.com/upload/drive/v3/files?uploadType=resumable", {
    ...
    body: JSON.stringify({
      ...
      description: "encrypted with cryptogle.io",
    }),
    headers: {
      Authorization: `Bearer ${token}`,
